...
const float SceneMainMenu::ROTATION_SPEED = 7.0f;
const float SceneMainMenu::MAX_ROTATION_SPEED = 7.0f;
const float SceneMainMenu::GRAVITY_CONSTANT = 3.0f;
...
float SceneMainMenu::CalculateAdditionalForce(GameObject* go1, GameObject* go2)
{
	float radiusSquared = go1->pos.DistanceSquared(go2->pos);
	return (GRAVITY_CONSTANT * go1->mass * go2->mass) / radiusSquared;
}

void SceneMainMenu::Wrap(float& val, float bound)
{
	if (val < 0)
	{
		val += bound;
