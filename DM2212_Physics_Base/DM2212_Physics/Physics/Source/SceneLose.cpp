...
const float SceneLose::ROTATION_SPEED = 7.0f;
const float SceneLose::MAX_ROTATION_SPEED = 7.0f;
const float SceneLose::GRAVITY_CONSTANT = 3.0f;
...
void SceneLose::RenderGO(GameObject *go)
{
	//Exe 4
	switch (go->type)
	{
...
float SceneLose::CalculateAdditionalForce(GameObject* go1, GameObject* go2)
{
	float radiusSquared = go1->pos.DistanceSquared(go2->pos);
	return (GRAVITY_CONSTANT * go1->mass * go2->mass) / radiusSquared;
}

void SceneLose::Wrap(float& val, float bound)
{
	if (val < 0)
	{
		val += bound;
